...
class Scraper extends Request_Client_Curl {

	public function humanize($withCookies = FALSE)
	{
		if ($withCookies)
		{
			$cookieName = 'www.fedex.com';

			$this->options(CURLOPT_COOKIEJAR,  APPPATH.'cache/'.$cookieName.'.cookie');
			$this->options(CURLOPT_COOKIEFILE, APPPATH.'cache/'.$cookieName.'.cookie');
		}

		$this->options(CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.146 Safari/537.36');
		$this->options(CURLOPT_REFERER, 'http://www.dhl.is/en/express/tracking.html');
		$this->options(CURLOPT_HEADER, 'Accept-Encoding: gzip,deflate,sdch');
	}

	public function _send_message(Request $request, Response $response)
	{
		// Call parent function
		$response = parent::_send_message($request, $response);

		$dom = new HTML_Parser_HTML5($response->body());
		$response->html = $dom->root;
